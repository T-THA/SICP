...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    while num_rolls>0:
        k = dice()
        if k % 2==current_score % 2:
            add_score_parity +=1
        else:
            add_score_parity = add_score_parity
        if k ==1:
            judge = 0
            add_score_one = 1
        else:
            add_score_dice += k
        num_rolls -=1
    add_score = add_score_one + add_score_parity + judge *add_score_dice
    return add_score
    # END PROBLEM 1
...
def swine_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    swine_judge_last = 3**(player_score+opponent_score)
    swine_judge_first = 3**(player_score+opponent_score) % 10
    while swine_judge_last > 10:
        swine_judge_last = swine_judge_last //10
    swine_judge_last = swine_judge_last % 10
    return swine_judge_first==swine_judge_last
    # END PROBLEM 4
