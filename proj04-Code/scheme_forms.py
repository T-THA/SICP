...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        new_expr=expressions.rest
        #pair(x, pair(pair(+......)，nil))
        env.define(signature,scheme_eval(new_expr.first,env))
        return signature
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        tmp_bind_pair=Pair(signature.rest,expressions.rest)
        bind_lambda_procedure=do_lambda_form(tmp_bind_pair,env)
        #在当前frame要定义出来 名字 值
        env.define(signature.first,bind_lambda_procedure)
...
def do_and_form(expressions, env):
    ...
    if expressions.rest is not nil:
        tmp_and_eval=scheme_eval(expressions.first,env)
        if is_scheme_false(tmp_and_eval):
            return False
        else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    if expressions.rest is not nil:
        tmp_or_eval=scheme_eval(expressions.first,env)
        if is_scheme_true(tmp_or_eval):
            return tmp_or_eval
        else:
